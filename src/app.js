...
var UIController = (function () {
  ...
  var display = function (id, type) {
    if (!document.querySelector("#" + id).hasChildNodes()) {
      document
        .querySelector("#" + id)
        .insertAdjacentHTML("beforeend", html[type]);
    }
  };
  return {
    DOM: function () {
  ...
  return {
    ...
    winner: {
      gameOver: function (activePlayer) {
        if (activePlayer === 0) {
          return (
            player0.i.threeValuesSame() ||
            player0.j.threeValuesSame() ||
            diagnol1check(player0.i, player0.j) ||
            diagnol2check(player0.i, player0.j)
          );
        } else if (activePlayer === 1) {
          return (
            player1.i.threeValuesSame() ||
            player1.j.threeValuesSame() ||
            diagnol1check(player1.i, player1.j) ||
            diagnol2check(player1.i, player1.j)
          );
        }
      },
    },
      ...
      for (var j = 0; j < this.length; j++) {
        if (this[i] === this[j]) {
          ...
          if (i !== j) {
            ...
            if (
              (nextArray[i] == 0 && nextArray[j] == 1) ||
              (nextArray[i] == 1 && nextArray[j] == 0)
            ) {
              returnValueB = 2;
              //console.log(returnValueB);
            } else if (
              (nextArray[i] == 1 && nextArray[j] == 2) ||
              (nextArray[i] == 2 && nextArray[j] == 1)
            ) {
              returnValueB = 0;
              //console.log(returnValueB);
            } else if (
              (nextArray[i] == 0 && nextArray[j] == 2) ||
              (nextArray[i] == 2 && nextArray[j] == 0)
            ) {
              returnValueB = 1;
              //console.log(returnValueB);
            }
...
var AppController = (function (UICtrl, gLogic, cLogic) {
  ...
  var setUpEvents = function () {
    document
      .querySelector(DOMStrings.newGame)
    ...
    document
      .querySelector(DOMStrings.board)
          ...
          if (itemId) {
            ...
            if (gLogic.winner.gameOver(activePlayer)) {
              ...
              if (computerPlaying) {
                ...
                cLogic.setCurrentPlayer(
                  gLogic.player.player1(),
                  gLogic.player.player2()
                );
    ...
    document
      .querySelector(DOMStrings.computer)
      .addEventListener("click", function () {
        UICtrl.computer();
  ...
  return {
    init: function () {
      initiateComponents(), setUpEvents();
    },
  };
